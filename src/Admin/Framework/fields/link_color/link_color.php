 ...
 *
 */
if ( ! class_exists( 'Themeatelier_Core_Field_link_color' ) ) {
  class Themeatelier_Core_Field_link_color extends Themeatelier_Core_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'themeatelier-core' ),
        'hover'    => esc_html__( 'Hover', 'themeatelier-core' ),
        'active'   => esc_html__( 'Active', 'themeatelier-core' ),
        'visited'  => esc_html__( 'Visited', 'themeatelier-core' ),
        'focus'    => esc_html__( 'Focus', 'themeatelier-core' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="themeatelier-core--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="themeatelier-core-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
